...
export const Introduction: FC<IntroductionProps> = ({ className }) => {
  ...
  return (
    <Container className={className}>
      <div className="rounded-2xl px-6 pb-20 pt-24 ring-1 ring-zinc-200 dark:ring-zinc-800">
        <div className="mx-auto max-w-[620px] text-center">
          ...
          <div className="mt-8 flex justify-center gap-6">
            {social.map(({ href, label, icon: SocialIcon }) => (
              <Link
                className="h-5 w-5 rounded-md p-0.5 text-zinc-500 outline-none transition hover:text-zinc-800 focus-visible:shadow-focus dark:text-zinc-400 dark:hover:text-zinc-200"
                key={href}
                href={href}
                aria-label={label}
              >
